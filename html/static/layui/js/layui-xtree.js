...
function layuiXtree(options) {
    ...
    this._elem = options.elem;  //id名称
    this._readonly = options.readonly != null ? options.readonly : false;  //只读
    this._form = options.form; //layui from对象
    this._domStr = "";  //结构字符串
    this._isopen = options.isopen != null ? options.isopen : false;
    console.log(this._isopen);
    this._selected = options.selected != null ? options.selected : [];
...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (i in d) {
            ...
            var other_str=" ";
            if(_this._readonly){
                input_str=( _this._readonly?'   readonly disabled="disabled"   ' :'');
                other_str=( _this._readonly?'   readonly disabled="disabled"   ' :'')
                +( _this._readonly?'    ' :' name="'+_this._elem+'[]" ');
            }else{
                other_str=( d[i].readonly==1?'   readonly disabled="disabled"   ' :'')
                    +( d[i].readonly==1?'    ' :' name="'+_this._elem+'[]" ');
            }
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" '
                +( _this.in_array(d[i].value,_this._selected)?' checked="" ' :'')

                + other_str

                + xtree_isend + ' value="' + d[i].value + '" title="' + d[i].title
                + '" is_only_read="'+( _this._readonly || d[i].readonly==1?'1' :'0') +'" '
...
//生产结构
layuiXtree.prototype.in_array = function (search,array) {
    for(var i in array){
        if(array[i]==search){
            return true;
        }
    ...
    if(!_this._readonly){
        _this._form.on('checkbox(xtreeck)', function (da) {
            ...
            for (var i = 0; i < xtree_chis.length; i++) {
                if(xtree_chis[i].getElementsByClassName('layui-xtree-checkbox')[0].getAttribute('is_only_read')=="0"){
...
layuiXtree.prototype.ParendCheck = function (ckelem) {
    ...
    if (xtree_p.getAttribute('class') == 'layui-xtree-item') {
        ...
        // console.log(xtree_p.getElementsByClassName('layui-xtree-checkbox')[0].getAttribute('is_only_read'));
        if(xtree_p.getElementsByClassName('layui-xtree-checkbox')[0].getAttribute('is_only_read')=="0" ){

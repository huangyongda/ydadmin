...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        var TabId = that.getTabId(data.href);

        if (TabId !== 0) {
            element.tabDelete(ELEM.tabFilter, TabId);
            tabIndex = that.exists(data.href);
        }
        var waitLoadIndex;
        if (tabIndex === -1) {
            if (_config.openWait) {
        ...
        if (_config.contextMenu) {
            ...
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                ul += '<li data-target="newTabOpen" title="新窗口打开当前选项卡"><i class="fa fa-clone" aria-hidden="true"></i>新窗口打开</li>';
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                ...
                                case 'closeCurrent': //关闭当前
                                    layer.confirm('确定要关闭吗?', { icon: 3, title: '系统提示' }, function (index) {
                                        if (clickIndex !== 0) {
                                ...
                                case 'closeOther': //关闭其他
                                    layer.confirm('确定要关闭其他吗?', { icon: 3, title: '系统提示' }, function (index) {
                                    ...
                                    break;
                                case 'closeAll': //全部关闭
                                    layer.confirm('确定要全部关闭吗?', { icon: 3, title: '系统提示' }, function (index) {
